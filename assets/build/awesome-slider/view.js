...
(function () {
    ...
    function initAwesome_BlockSliders() {
      $('.awesome_block_banner__slider .slider:not(.slick-initialized)').each(function () {
        ...
        $this.on('afterChange', function (event, slick, currentSlide) {
          ...
          $(slick.$slides[currentSlide]).find('.slide__img img').addClass('zooming');

          // Remove zooming from other slides
          $(slick.$slides).not(slick.$slides[currentSlide]).find('.slide__img img').removeClass('zooming');
        });
      });

      // Handle arrow click animations
      $(document).off('click', '.slick-nav').on('click', '.slick-nav', function (e) {
        var $arrow = $(this);
  ...
  if (typeof elementor !== 'undefined') {
    elementor.hooks.addAction('panel/open_editor/widget/awesome-slider', function () {
