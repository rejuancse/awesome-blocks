...
export default function Edit({ attributes, setAttributes }) {
    ...
    return (
        <>
            <InspectorControls>
                <PanelBody title={__('Layout Settings', 'zepblocks' )} initialOpen={true}>
                    <RangeControl
                    ...
                    />
                    <RangeControl
                        label={__('Products per Page', 'zepblocks' )}
                ...
                <PanelBody title={__('Product Query', 'zepblocks' )} initialOpen={false}>
                    <PanelRow>
                        <SelectControl
                            ...
                            options={[
                                ...
                                { label: __('Price', 'zepblocks' ),      value: 'price' },
                                { label: __('Popularity', 'zepblocks' ), value: 'popularity' },
                                { label: __('Rating', 'zepblocks' ),     value: 'rating' },
                    ...
                    <PanelRow>
                        <div style={{ width: '100%' }}>
                            <label style={{ display: 'block', marginBottom: '8px', fontWeight: '500' }}>
                                {__('Filter by Categories', 'zepblocks' )}
                            </label>
                            <SelectControl
                                value=""
                                ...
                                help={
                                    selectedCategories.length > 0
                                        ? __('Selected:', 'zepblocks' ) + ' ' + selectedCategoryNames.join(', ')
                                        : __('Select categories to filter products', 'zepblocks' )
                                }
                            />
                            {selectedCategories.length > 0 && (
                ...
                <PanelBody title={__('Display Options', 'zepblocks' )} initialOpen={false}>
                    <ToggleControl label={__('Show Category', 'zepblocks' )}     checked={showCategory}   onChange={() => setAttributes({ showCategory:   !showCategory })} />
                    <ToggleControl label={__('Show Rating', 'zepblocks' )}       checked={showRating}     onChange={() => setAttributes({ showRating:     !showRating })} />
                    <ToggleControl label={__('Show Price', 'zepblocks' )}        checked={showPrice}      onChange={() => setAttributes({ showPrice:      !showPrice })} />
                    <ToggleControl label={__('Show Add to Cart', 'zepblocks' )}  checked={showAddToCart}  onChange={() => setAttributes({ showAddToCart:  !showAddToCart })} />
                    <ToggleControl label={__('Show Badges', 'zepblocks' )}       checked={showBadges}     onChange={() => setAttributes({ showBadges:     !showBadges })} />
                    {showBadges && (
                        <SelectControl
                            label={__('Badge Position', 'zepblocks' )}
                            value={badgePosition}
                            options={[
                                { label: __('Top Left',     'zepblocks' ), value: 'top-left' },
                                { label: __('Top Right',    'zepblocks' ), value: 'top-right' },
                                { label: __('Bottom Left',  'zepblocks' ), value: 'bottom-left' },
                                { label: __('Bottom Right', 'zepblocks' ), value: 'bottom-right' },
                            ]}
            ...
            <div {...blockProps}>
                {!hasResolved ? (
                    ...
                    <div style={{ padding: '40px', textAlign: 'center', background: '#f9f9f9', borderRadius: '8px' }}>
                        <p>{__('No products found. Add some WooCommerce products first.', 'zepblocks' )}</p>
                    </div>
                ) : (
                    <div className={`zepblocks-products-list zepblocks-columns-${columns}`}>
                        <div className="zepblocks-row">
                            {products.map((product) => {
                                ...
                                return (
                                    <div key={product.id} className={`zepblocks-col zepblocks-col-${12 / columns}`}>
                                        <div className="zepblocks-product-card">
                                            <div className="zepblocks-product-image-wrapper">
                                                <a href={productLink} className="zepblocks-product-image" target="_blank" rel="noopener noreferrer">
                                                    <img src={imageUrl} alt={product.name || product.title?.rendered || ''} />
                                                </a>

                                                {showBadges && discountPct > 0 && (
                                                    <div className={`zepblocks-product-badges zepblocks-badge-${badgePosition}`}>
                                                        <span className="zepblocks-discount-badge">-{discountPct}%</span>
                                                    </div>
                                                )}
                                            </div>

                                            <div className="zepblocks-product-details">
                                                {/* Category */}
                                                {categoryHtml && (
                                                    <div className="zepblocks-product-category">
                                                        <span>{categoryHtml}</span>
                                                    </div>
                                                )}

                                                {/* Title */}
                                                <h3 className="zepblocks-product-title">
                                                    <a href={productLink} target="_blank" rel="noopener noreferrer">
                                                        {product.name || product.title?.rendered || __('Untitled Product', 'zepblocks' )}
                                                    </a>
                                                </h3>

                                                {/* Rating */}
                                                {showRating && rating > 0 && (
                                                    <div className="zepblocks-product-rating">
                                                        <div className="star-rating" title={`Rated ${rating} out of 5`}>
                                                            <span style={{ width: `${ratingWidth}%` }}>
                                                                <strong>{rating}</strong> out of <span>5</span>
                                                            </span>
                                                        </div>
                                                        <div className="zepblocks-review-count">({count})</div>
                                                    </div>
                                                )}

                                                {/* Price */}
                                                {showPrice && price && (
                                                    <div className="zepblocks-product-price">
                                                        {onSale && regularPrice && (
                                                            <del className="zepblocks-regular-price">${regularPrice}</del>
                                                        )}
                                                        <span className={onSale ? 'zepblocks-sale-price' : ''}>
                                                ...
                                                {showAddToCart && (
                                                    <div className="zepblocks-add-to-cart">
                                                        <span>{__('Add to cart', 'zepblocks' )}</span>
                                                    </div>
                                                )}
                                            </div>
                                        </div>
