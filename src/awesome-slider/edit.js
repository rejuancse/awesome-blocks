...
export default function Edit({ attributes, setAttributes }) {
    ...
    return (
        <>
            <InspectorControls>
                <PanelBody title={__('Slider Settings', 'awesome-blocks')} initialOpen={true}>
                    <PanelRow>
                        <ToggleControl
                    ...
                    {autoplay && (
                        <PanelRow>
                            <TextControl
                                label={__('Autoplay Speed (ms)', 'awesome-blocks')}
                                type="number"
                                value={autoplaySpeed}
                    ...
                    <PanelRow>
                        <TextControl
                            label={__('Animation Speed (ms)', 'awesome-blocks')}
                            type="number"
                    ...
                    <PanelRow>
                        <ToggleControl
                            label={__('Show Arrows', 'awesome-blocks')}
                            checked={showArrow}
                    ...
                    <PanelRow>
                        <ToggleControl
                            label={__('Show Dots', 'awesome-blocks')}
                            checked={showDots}
                ...
                <PanelBody title={__('Manage Slides', 'awesome-blocks')} initialOpen={false}>
                    {slides.map((slide, index) => (
                        <div key={slide.id} style={{ marginBottom: '20px', padding: '15px', background: '#f0f0f0', borderRadius: '4px' }}>
                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '10px' }}>
                                <strong>{__('Slide', 'awesome-blocks')} {index + 1}</strong>
                                {slides.length > 1 && (
                                    <Button
                            ...
                            <TextControl
                                label={__('Top Title', 'awesome-blocks')}
                            ...
                            />
                            <SelectControl
                                label={__('Button Style', 'awesome-blocks')}
                                value={slide.buttonStyle}
                                options={[
                                    { label: __('Success (Green)', 'awesome-blocks'), value: 'success' },
                                    { label: __('Light (White)', 'awesome-blocks'), value: 'light' },
                                ]}
                                onChange={(value) => updateSlide(index, 'buttonStyle', value)}
                            />
                            <SelectControl
                                label={__('Content Position', 'awesome-blocks')}
                ...
                {slides.length === 0 ? (
                    <div className="as-placeholder">
                        <p style={{ textAlign: 'center', padding: '40px', background: '#f9f9f9', borderRadius: '4px' }}>
                            {__('No slides yet. Add slides from the sidebar settings.', 'awesome-blocks')}
                        </p>
                    </div>
                ) : (
